...
export class UserService {
	...
	getUsers() {
		return this.http.get<UserInterface[]>(api).pipe(catchError(this.errorHandler));
	}
	...
	deleteUser(user: string): void {
		...
		if (userData) {
			...
			const index = newUsers.findIndex((u: UserInterface) => JSON.stringify(u) === user);
			if (index >= 0) {
	...
	updateUser(cpf: string, user: UserInterface): void {
		...
		if (userData) {
			...
			const updatedUsers = users.map((u: UserInterface) => (u.cpf === cpf ? user : u));
